...
noisefolderpath = "dataset/other_noise"     # "OtherNoise_leftovers/train_noises"    # NB don't include final slash
speechfolderpath = "dataset/test-clean-withoutnoise"            # "LibriSpeech/test-clean"                # NB don't include final slash
finalfolder = "other_noise_final/test"         # "OtherNoise_final/test_selfmade"               # NB don't include final slash
...
for clean_wav in speech_list:
   ...
   ### Get info about snr and noise_wavname (which form a combined string in temp_noise_snr_list, so the correct part has to be selected)
    snr_match = re.search(r"(-?\d+)$", random_item)
    noise_wav_match = re.search(r"(.*_16kHz\.wav)-?\d+$", random_item)

    if snr_match and noise_wav_match:
        snr = snr_match.group(1)
        noise_wav = noise_wav_match.group(1)
    else:
        print(f"Error parsing SNR or noise filename from: {random_item}")
        print(f"Expected format: <filename>_16kHz.wav<snr>")
        continue
